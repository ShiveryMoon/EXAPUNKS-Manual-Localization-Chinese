...
* 如果某个EXA实例读取空的M寄存器，它将会被阻塞；反之，如果某个EXA实例想要将值写入已有值的M寄存器，它也会被阻塞，直到另一个EXA实例将值从它那端的M寄存器中取出。因此，你不需要掐准时机去进行EXA间通信。
...
* 你可以通过TEST指令来比较一个寄存器和另一个寄存器(或一个整形)是否相等，比如比较X是否等于38(TEST X = 38)，如果结果为True，那么**T寄存器**会被置1，否则，**T寄存器**会被置0。
...
### 除以0
&emsp;&emsp;你不能执行除以0操作，基本的数学常识。别问，记住就行了。
### 对字符串进行运算
&emsp;&emsp;你不能对字符串进行加减乘除，数学运算只能用于数字。
### 非法访问F寄存器
&emsp;&emsp;如果一个手中没有文件的EXA尝试进行读写操作，则它会发生错误并终止。不要忘了先把文件拿起来。
### 非法访问硬件寄存器
&emsp;&emsp;和上一条一样。当你准备对硬件进行读写时，确保你的EXA和硬件寄存器处于同一主机中。  
...
### 非法文件访问  
&emsp;&emsp;当你想寻找某个特定文件时，如果发生了该错误，则说明该文件不存在。  
### 非法穿越链接
&emsp;&emsp;当某个EXA希望通过不存在的链接时，该EXA会被终止，但至少你知道了该链接不存在。
&emsp;&emsp;讲真，不要担心你的EXA会发生错误，虽然可能看上去它们很可爱，但它们存在的目的就是不停地交换、计算、生成并死亡，你要学会利用这一点。
