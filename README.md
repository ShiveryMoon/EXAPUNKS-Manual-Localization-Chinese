...
### 硬件寄存器
&emsp;&emsp;有些主机拥有特殊的硬件寄存器(hardware registers)。每个硬件寄存器代表了一个连接到当前主机的外部设备。不同的外设，硬件寄存器不同，它们的读写限制也不同。这些硬件寄存器也使用整形或字符串。它们的标识规则如下：一个井号后面跟着4个数字，比如`#POWR`或`#ENAB`。你可以在你的代码里直接使用这些标识。
