...
&emsp;&emsp;按下step按钮，以行为单位来调试你的代码。当你感觉差不多了以后，按下run按钮来执行全部代码。当你确认自己的代码没问题以后，按下forword来运行所有的case，直到结束。恭喜，你完成了你的首次EXA编程。
### 教程2：文件读写
...
&emsp;&emsp;还有一点。当读操作或者写操作完成以后，文件指针将会自动后移一个单位。这个特性有时是很方便的，但有时却并不。
### 教程3：EXA间通信
...
### 教程4：条件与循环
&emsp;&emsp;你的最后一个任务，是创建一个文件并输入特定的内容。做到这点需要你使用LOOP指令，它可以让你循环执行代码，直到达到某特定条件并退出循环。给你一些建议：
...
* 下面给出一个循环的例子。假设你想向某个文件中写入9999，一共写10次，你可以用寄存器X来记录当前的已写入次数：
